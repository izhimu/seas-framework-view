...
const td = { col: 1, row: 1, hide: false, columns: [] };
const tr = [td, td];
  ...
  setup(props, context) {
    ...
    const showRightColMenu = computed(() => {
      ...
      if (state.showContextMenu) {
        const { col } =
          data.value.options.layoutArray[state.currentRowIndex][
    ...
    const showDownRowMenu = computed(() => {
      if (state.showContextMenu) {
        ...
        let td;
        if (
          typeof data.value.options.layoutArray[state.currentRowIndex + row] !==
          "undefined"
        ) {
          td =
            data.value.options.layoutArray[state.currentRowIndex + row][
              state.currentColIndex
            ];
        }
        return td && td.row < 2 && td.col < 2 && !td.hide;
    ...
    const handlerRightCol = () => {
      ...
      if (row > 1) {
        for (let i = 0; i < row; i++) {
          data.value.options.layoutArray[state.currentRowIndex + i][
            state.currentColIndex + col
          ].hide = true;
          data.value.options.layoutArray[state.currentRowIndex][
            state.currentColIndex
