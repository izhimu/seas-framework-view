<template>
  <n-form v-if="data" :key="data.key" layout="vertical">
    <n-form-item v-if="data.type !== 'grid'" label="字段标识">
      <n-input v-model:value="data.model" />
    </n-form-item>

    <n-form-item
      v-if="data.type !== 'grid' && data.options.showLabel"
      label="标题"
    >
      <n-input v-model:value="data.label" />
    </n-form-item>

    <n-form-item v-if="hasKey('width')" label="宽度">
      <n-input v-model:value="data.options.width" />
    </n-form-item>

    <n-form-item v-if="hasKey('height')" label="高度">
      <n-input v-model:value="data.options.height" />
    </n-form-item>

    <n-form-item v-if="hasKey('placeholder')" label="占位内容">
      <n-input v-model:value="data.options.placeholder" />
    </n-form-item>

    <component
      :is="data.type"
      v-if="checkComponent(data.type)"
      :select="data"
    />
    <template v-if="hasKey('click') || hasKey('change') || hasKey('input')">
      <h4>事件处理</h4>
      <n-form-item v-if="hasKey('change')" label="change事件">
        <n-button type="info" dashed @click="editEventCode('change')"
          >编辑代码</n-button
        >
      </n-form-item>
      <n-form-item v-if="hasKey('click')" label="click事件">
        <n-button type="info" dashed @click="editEventCode('click')"
          >编辑代码</n-button
        >
      </n-form-item>
    ...
    <template v-if="hasKey('rules')">
      ...
      <n-form-item label="校验类型">
        <n-select
          v-model:value="data.options.rules.type"
  ...
  >
    <n-alert type="info" style="margin-bottom: 10px"
      ><n-space>
        使用示例
        <n-button text type="primary" @click="setTestEvent('dynamicDisabled')"
          >动态禁用/启用</n-button
        >
        <n-button text type="primary" @click="setTestEvent('dynamicSetValue')"
          >动态赋值</n-button
        >
        <n-button text type="primary" @click="setTestEvent('dynamicRequired')"
          >必填项修改</n-button
        >
        <n-button text type="primary" @click="setTestEvent('dynamicHideLabel')"
          >Label动态显隐</n-button
        >
    ...
    >
    <div class="code-bord">
      function {{ eventType }}(e,view,form){
