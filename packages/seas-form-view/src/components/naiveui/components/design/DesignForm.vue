<template>
  <div class="widget-form-container">
    ...
    <n-form
      ...
      :label-width="widgetForm.config.labelWidth"
          ...
          <transition-group name="fade" tag="div">
            ...
            <template v-else-if="element.type === 'table'">
              ...
              <n-table
                ...
                :key="element.key"
                :class="
                  (widgetFormSelect?.key === element.key ? 'active' : '') +
                  ' table-' +
                ...
                <tbody>
                  ...
                  >
                    <td
                      ...
                      @contextmenu.prevent="
                        rightClick($event, element, colIndex, tdIndex)
                      "
                    >
                      <Draggable
                        class="widget-col-list"
                        ...
                        :list="td.list"
                        @add="
                          handleTdMoveAdd($event, element, colIndex, tdIndex)
                        "
                      >
                        <template #item="{ element, index }">
                          <transition-group name="fade" tag="div">
              ...
              >
                <ul style="list-style-type: none">
                  <li v-if="showRightColMenu" @click="handlerRightCol">
                    向右合并单元格
                  </li>
                  <li v-if="showDownRowMenu" @click="handlerDownRow">
                    向下合并单元格
                  </li>
                  <li v-if="showResetTableMenu" @click="handlerResetTable">
                    拆分单元格
                  </li>
                  <li v-if="showDeleteRowMenu" @click="handlerDeleteRow">
                    删除行
                  </li>
  ...
  setup(props, context) {
    ...
    const showRightColMenu = computed(() => {
      if (state.showContextMenu) {
        const { col } =
          state.currentElement.columns[state.currentRowIndex][
    ...
    const showDownRowMenu = computed(() => {
      if (state.showContextMenu) {
        ...
        let td;
        if (
          typeof state.currentElement.columns[state.currentRowIndex + row] !==
          "undefined"
        ) {
          td =
            state.currentElement.columns[state.currentRowIndex + row][
              state.currentColIndex
            ];
        }
        return td && td.row < 2 && td.col < 2 && !td.hide;
    ...
    const handlerRightCol = () => {
      ...
      if (row > 1) {
        for (let i = 0; i < row; i++) {
          state.currentElement.columns[state.currentRowIndex + i][
            state.currentColIndex + col
          ].hide = true;
          state.currentElement.columns[state.currentRowIndex][
            state.currentColIndex
    ...
    const handlerAppendCol = () => {
      const lastTrLen =
        state.currentElement.columns[state.currentElement.columns.length - 1]
          .length;
      const trs = [];
