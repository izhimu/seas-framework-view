<template>
  ...
  <n-form-item label="面板的弹出位置">
    <n-select
      v-model:value="data.options.placement"
  ...
  <n-form-item label="选项">
    <div style="width: 100%">
      <n-radio-group v-model:value="data.options.remote" button-style="solid">
      ...
      <template v-else>
        <div style="margin-top: 10px">
          <n-button type="primary" @click="addTreeData()">添加</n-button>
          <n-data-table
            id="selectTree-target"
            style="margin-top: 10px"
            ...
            <n-drawer-content>
              ...
              <template #footer>
                <n-space>
                  ...
                  <n-button type="primary" @click="submitTreeData"
                    >确定</n-button
                  >
                </n-space>
              </template>
    ...
    const state = reactive({
      ...
      treeDataColumns: treeDataColumns({
        ...
        edit(rowData, rowIndex) {
          ...
          state.treeData = { ...rowData };
          state.showTreeDataDriwer = true;
          state.showTreeDataType = "edit";
        },
        remove(rowData, rowIndex) {
          if (rowData.parentKey) {
            const parentTreeData = getParentTreeInfo(rowData.parentKey);
            ...
            const index = getTreeFirstIndex(rowData.key);
        ...
        if (!errors) {
          if (state.parentTreeData) {
            if (state.showTreeDataType === "add") {
              if (!state.parentTreeData.children)
                state.parentTreeData.children = [];
