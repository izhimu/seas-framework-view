<template>
  <n-form-item label="常用宽高">
    <n-space style="margin-left: 3px">
      <n-tag
        v-for="(item, index) in whTemplateList"
        :key="index"
        class="hoverTag"
        type="primary"
  ...
  <n-form-item label="表格属性">
    <n-space item-style="display: flex;" align="center">
      <n-checkbox v-model:checked="data.options.bordered"
        >是否显示边框
      </n-checkbox>
      <n-checkbox v-model:checked="data.options.bottomBordered"
        >表格底部是否有边框
      </n-checkbox>
      <n-checkbox v-model:checked="data.options.singleColumn"
        >是否不设定行的分割线
      </n-checkbox>
      <n-checkbox v-model:checked="data.options.singleLine"
        >是否不设定列的分割线
      </n-checkbox>
      <n-checkbox v-model:checked="data.options.striped"
        >是否使用斑马线条纹
      </n-checkbox>
    </n-space>
  </n-form-item>
